...
object AST {
  ...
  def accumulatePat[T] ( p: Pattern, f: Pattern => T, acc: (T,T) => T, zero: T ): T =
    p match {
      ...
      case CallPat(_,ps)
        => ps.map(f(_)).fold(zero)(acc)
      case MethodCallPat(o,_,ps)
        => ps.map(f(_)).fold(f(o))(acc)
      case _ => zero
    }
  ...
  def toExpr ( p: Pattern ): Expr
      = p match {
        ...
        case BooleanPat(n)
          => BoolConst(n)
        case CallPat(s,ps)
          => Call(s,ps.map(toExpr))
        case MethodCallPat(mp,m,null)
          => MethodCall(toExpr(mp),m,null)
        case MethodCallPat(mp,m,ps)
          => MethodCall(toExpr(mp),m,ps.map(toExpr))
        case _ => Tuple(Nil)
      }
}
