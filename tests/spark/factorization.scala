...
object Factorization extends Serializable {
  ...
  def main ( args: Array[String] ) {
    ...
    var P = new BlockMatrix(Pm,N,N)
    var Q = new BlockMatrix(Qm,N,N)

    val RR = (n,m,Rm.map{ case ((i,j),a) => ((i,j),a.toArray) })
    var PP = (n,d,Pm.map{ case ((i,j),a) => ((i,j),a.toArray) })
    var QQ = (m,d,Qm.map{ case ((i,j),a) => ((i,j),a.toArray) })
    ...
    def testFactorizationDiablo(): Double = {
      ...
      var EE = QQ
      val E2 = ar("""
                 tiled(n,m)[ (+/v)[i,j] | p[i,k] <- PP, q[kk,j] <- QQ,
      ...
      PP = ar("""
             tiled(n,d)[ (^/v)[i,k]
                       | e[i,j] <- EE, q[k,jj] <- QQ, jj == j,
                         v = 2*0.002*e*q, group by (i,k) ]
             """)
      QQ = ar("""
             tiled(m,d)[ (^/v)[k,j]
                       | e[i,j] <- EE, p[ii,k] <- PP, ii == i,
                         v = 2*0.002*e*p, group by (k,j) ]
             """)
      val x = EE._3.count+PP._3.count+QQ._3.count
      (System.currentTimeMillis()-t)/1000.0
    }
